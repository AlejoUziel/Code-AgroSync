...
const estadoConfig: Record<string, { badge: string; dot: string }> = {
  "Disponible": { badge: "bg-[#F0F5EA] text-[#8EBF24] border-0", dot: "bg-[#8EBF24]" },
...
const categoriaColor: Record<string, string> = {
  "Fertilizante": "bg-[#BEE86B]/20 text-[#6E9A1A]",
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            <div key={s.label} className={`bg-white rounded-xl border p-4 ${s.alert ? "border-amber-200" : "border-[#E2EDD6]"}`}>
            ...
            <table className="w-full">
              ...
              <tbody>
                {inventario.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className="border-b border-[#F0F5EA] hover:bg-[#F9FBF6] transition-colors group">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <div className="w-6 h-6 rounded-md bg-[#8EBF24]/10 flex items-center justify-center">
                            <WarehouseIcon size={11} className="text-[#8EBF24]" />
                          </div>
                      ...
                      <td className="px-4 py-3 w-28">
                        <Progress
                          value={pct}
                          className={`h-1.5 bg-[#E2EDD6] ${item.estado === "Agotado" ? "[&>div]:bg-red-500" : item.estado === "Stock Bajo" ? "[&>div]:bg-amber-500" : "[&>div]:bg-[#8EBF24]"}`}
                        />
                      </td>
