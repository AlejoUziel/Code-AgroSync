...
const estadoConfig: Record<string, string> = {
  "En Progreso": "bg-secondary text-primary border-0",
...
const etapaColor: Record<string, string> = {
  ...
  "Llenado": "bg-accent/30 text-[var(--primary-dark)]",
};
        ...
        <div className="bg-card rounded-xl border border-border">
                ...
                {cultivos.map((c) => (
                  <tr key={c.id} className="border-b border-secondary hover:bg-background transition-colors group">
                    ...
                    <td className="px-4 py-3 w-36">
                      <div className="flex items-center gap-2">
                        <Progress value={Math.round((c.dias / c.ciclo) * 100)} className="h-1.5 flex-1 bg-card" />
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-border">
            ...
            <div className="flex items-center gap-1">
              {[1, 2, 3, "...", 26].map((p, i) => (
                <button key={i} className={`w-7 h-7 rounded-md text-xs font-body transition-colors ${p === 1 ? "bg-primary text-white" : "text-[#6B7280] hover:bg-secondary"}`}>{p}</button>
              ))}
            </div>
          </div>
        </div>
