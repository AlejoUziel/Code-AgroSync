...
const estadoUsuario: Record<string, { badge: string; dot: string }> = {
  Activo: { badge: "bg-[#F0F5EA] text-[#8EBF24] border-0", dot: "bg-[#8EBF24]" },
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-5">
        {[
          { label: "Total Usuarios", value: usuarioDB.records.length, sub: "registrados", icon: <Users size={16} className="text-[#8EBF24]" /> },
          { label: "Activos Hoy", value: activeUsers, sub: "sesiones activas", icon: <Shield size={16} className="text-[#8EBF24]" /> },
          { label: "Empresas", value: empresaDB.records.length, sub: `${activeEmpresas} activas`, icon: <Building2 size={16} className="text-[#8EBF24]" /> },
          { label: "Roles Distintos", value: [...new Set(usuarioDB.records.map((u) => u.rol))].length, sub: "tipos de acceso" },
        ].map((s) => (
          <div key={s.label} className="bg-white rounded-xl border border-[#E2EDD6] p-4 flex items-start gap-3">
            {s.icon && (
              <div className="w-9 h-9 rounded-lg bg-[#F0F5EA] flex items-center justify-center shrink-0">
                {s.icon}
              </div>
            )}
            <div>
      ...
      <div className="bg-white rounded-xl border border-[#E2EDD6]">
        {/* Tabs + toolbar */}
        <div className="flex items-center gap-0 border-b border-[#E2EDD6] px-4 pt-3">
          {(["usuarios", "empresas"] as TabKey[]).map((t) => (
            <button
              ...
              className={cn(
                ...
                tab === t
                  ? "border-[#8EBF24] text-[#8EBF24] font-medium-body"
        ...
        {/* Toolbar */}
        <div className="flex flex-wrap items-center gap-2 p-4 border-b border-[#E2EDD6]">
          {/* Search */}
          <div className="flex items-center gap-2 bg-[#F9FBF6] rounded-lg px-3 py-2 border border-[#E2EDD6] flex-1 min-w-[200px] max-w-xs focus-within:border-[#8EBF24] transition-colors">
          ...
          {tab === "usuarios" && (
            <button
              ...
              className={cn(
                ...
                showFilters
                  ? "border-[#8EBF24] text-[#8EBF24] bg-[#F0F5EA]"
                  : "border-[#E2EDD6] text-[#6B7280] hover:border-[#8EBF24]/40"
              )}
            >
              <Filter size={13} />
              Filtros
              {(filterRol || filterEmpresaId || filterEstado) && (
                <span className="w-1.5 h-1.5 rounded-full bg-[#8EBF24]" />
              )}
            </button>
          )}
          ...
          <button
            ...
            className="p-2 rounded-lg border border-[#E2EDD6] text-[#9CA3AF] hover:text-[#8EBF24] hover:border-[#8EBF24]/40 transition-all"
          >
            <RefreshCw size={13} />
          </button>
          ...
          <button
            ...
            className="ml-auto flex items-center gap-2 px-4 py-2 rounded-lg bg-[#8EBF24] text-white text-xs font-medium-body hover:bg-[#6E9A1A] transition-colors"
          >
            <Plus size={13} />
        ...
        {tab === "usuarios" && showFilters && (
          <div className="flex flex-wrap gap-2 px-4 py-3 bg-[#F9FBF6] border-b border-[#E2EDD6]">
            <select
              value={filterRol}
              onChange={(e) => setFilterRol(e.target.value)}
              className="h-8 px-2 text-xs font-body rounded-lg border border-[#E2EDD6] bg-white outline-none focus:border-[#8EBF24] transition-colors"
            >
              ...
              <EmptyState
                ...
                action={
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[#8EBF24] text-white text-xs font-medium-body hover:bg-[#6E9A1A] transition-colors"
                  >
              ...
              <table className="w-full">
                ...
                <tbody>
                  {filteredUsuarios.map((u) => {
                    ...
                    return (
                      <tr key={u.id} className="border-b border-[#F0F5EA] hover:bg-[#F9FBF6] transition-colors group">
                        {/* Avatar + name */}
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2.5">
                            <Avatar className="h-8 w-8 shrink-0">
                              <AvatarFallback className="bg-[#8EBF24]/15 text-[#8EBF24] text-[10px] font-heading">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button
                              ...
                              className="p-1.5 rounded-md hover:bg-[#F0F5EA] text-[#9CA3AF] hover:text-[#8EBF24] transition-colors"
                            >
                              <Edit2 size={13} />
                            </button>
                            <button
                  ...
                  {filteredEmpresas.map((e) => {
                    ...
                    return (
                      <tr key={e.id} className="border-b border-[#F0F5EA] hover:bg-[#F9FBF6] transition-colors group">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2.5">
                            <div className="w-8 h-8 rounded-lg bg-[#8EBF24]/10 flex items-center justify-center shrink-0">
                              <Building2 size={14} className="text-[#8EBF24]" />
                            </div>
                            <div>
